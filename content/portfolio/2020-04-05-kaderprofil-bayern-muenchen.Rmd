...
```{r load libraries show, eval=FALSE}
...
Im nächten Schritt erstellen wir schon unsere fertige Grafik. Zuerst definieren wir die Farbpalette und manuell die Eigenschaften unserer x-Achse.
In der *ggplot*-Funktion legen wir fest, dass wir Minutenanteile (y-Achse) gegen jetziges Alter (x-Achse) in einem Scatterplot (*geom_point*) darstellen wollen um dann noch die obengenannten Schweife anzufügen (*ggforce::geom_link*).

```{r default show, eval=FALSE}
text_size <- 3
background_col <- "seagreen3"
link_col <- "red"
...
- Die Rohdaten für die obigen Charts findet ihr [hier](https://github.com/larsmaurath/keine-mathematik/blob/master/content/data/bayern_kader_profil_20200404.csv){target="_blank"}. 
- Natürlich könnt ihr auch selbst ähnliche Datensätze mit Information von [transfermarkt.de](https://www.transfermarkt.de/){target="_blank"} zusammenstellen.
